...
static u32 __init eap245_v1_test_sgmii_res_cal(void)
{
	...
	for (i = 0; i < 0x10; i++) {
		...
		t = __raw_readl(base + QCA956X_SGMII_SERDES_REG);
		...
		__raw_writel(t, base + QCA956X_SGMII_SERDES_REG);

		udelay(50);

		vco_after = __raw_readl(base + QCA956X_SGMII_SERDES_REG) &
...
static void __init eap245_v1_setup_qca956x_eth_serdes_cal(u32 sgmii_value)
{
	...
	while (!(__raw_readl(base + QCA956X_SGMII_SERDES_REG)
...
static void __init eap245_v1_setup_qca956x_sgmii(void)
{
	...
	t = __raw_readl(base + QCA956X_MR_AN_CONTROL_REG)
	...
	while (count++ < EAP245_V1_QCA956X_SGMII_LINK_MAX_RESET) {
		u8 status = (u8) __raw_readl(base + QCA956X_SGMII_DEBUG_REG);
		if (status == 0x1f || status == 0x10)
			break;

		__raw_writel(t | QCA956X_MR_AN_CONTROL_PHY_RESET,
				base + QCA956X_MR_AN_CONTROL_REG);
		udelay(100);
		__raw_writel(t, base + QCA956X_MR_AN_CONTROL_REG);
	}
